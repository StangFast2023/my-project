...
export default function T2P7_TableSummary({ data }) {
    // ดึงข้อมูลตามโครงสร้างที่คุณแจ้ง
    const part8 = data?.tab2?.part8 || {};
    ...
    const { roundColumns, grandTotal } = useMemo(() => {
        ...
        Object.values(part8).forEach(posTypeGroup => {
            Object.values(posTypeGroup).forEach(zone => {
                // หาจำนวนรอบสูงสุด
                if (zone.round_data) {
                    const r = Math.max(...Object.keys(zone.round_data).map(Number), 0);
                    if (r > maxR) maxR = r;
                }
                // สะสมยอดรวมทั้งหมด
                ...
                <table className="w-full text-left text-sm border-collapse min-w-[800px]">
                    ...
                    <tbody className="divide-y divide-gray-100">
                            ...
                            return (
                                <React.Fragment key={regId}>
                                    ...
                                    {Object.values(posTypes).map((zone) => {
                                        ...
                                        return (
                                            <tr key={zone.pos_type_id} className="hover:bg-gray-50">
                                                <td className={`w-[10%] px-6 py-3 pl-10 border-l-4 border-emerald-300 font-semibold ${zone.pos_type_id === "1" ? "bg-blue-50 text-blue-700" : zone.pos_type_id === "2" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
                                                    {zone.pos_type}
                                                </td>
                                                <td className="w-[10%] px-4 py-3 text-center font-bold text-gray-600 font-mono">{(Number(zone.total_list) || 0).toLocaleString()}</td>
                                                <td className="w-[10%] px-4 py-3 text-center font-bold text-emerald-600 font-mono">{(Number(zone.total_call) || 0).toLocaleString()}</td>
                                                {roundColumns.map(num => <td key={num} className="px-4 py-3 text-center border-l font-bold text-gray-600 font-mono">{(zone.round_data?.[num]?.total || 0).toLocaleString()}</td>)}
                                                <td className="w-[10%] px-4 py-3 text-center font-bold text-rose-600 font-mono">{(Number(zone.total_remain) || 0).toLocaleString()}</td>
                                            </tr>
                                        );
                                    })}
                                    <tr className="bg-gray-100 font-bold text-gray-800">
                                        ...
                                        {reg.rounds.map((v, i) => <td key={i} className="px-4 py-3 text-center border-l font-mono">{v.toLocaleString()}</td>)}
                        ...
                        <tr className="bg-gray-800 text-white font-bold">
                            ...
                            {roundColumns.map(n => <td key={n} className="px-4 py-4 text-center border-l border-gray-700 font-mono">{(grandTotal.rounds[n] || 0).toLocaleString()}</td>)}
