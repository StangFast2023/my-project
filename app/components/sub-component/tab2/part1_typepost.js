            ...
            <div className="grid grid-cols-1 gap-1 md:grid-cols-4 lg:grid-cols-4 lg:gap-4">
                ...
                {Object.entries(part7 || {}).filter(([key]) => key !== 't').map(([key, value]) => (
                    <div key={key} className={`flex-1 flex flex-col justify-center p-4 bg-gray-50 rounded-xl border-l-4 ${value.pos_type_id == 1 ? 'border-blue-500' : (value.pos_type_id == 2 ? 'border-green-500' : 'border-yellow-500')} my-2 shadow-xs`}>
                        <p className={` text-sm md:text-base lg:text-xl text-shadow-sm font-bold ${value.pos_type_id == 1 ? 'text-blue-500' : (value.pos_type_id == 2 ? 'text-green-500' : 'text-yellow-500')}  `}>{value.type_name}</p>
                        <div className="items-baseline gap-2 text-right">
                            <span className=" text-sm md:text-base lg:text-3xl font-bold text-gray-600">
                                <CountUp
