    ...
    return (
        <AnimatePresence>
            <motion.div
                ...
                initial={{ opacity: 0 }}   // ตอนเกิด
                animate={{ opacity: 1 }}   // ตอนแสดงผล
                exit={{ opacity: 0 }}      // ตอนหายไป
                transition={{ duration: 0.3 }} // เวลาที่ใช้
            >
                <motion.div
                    key="modal-content"
                    className="bg-gray-50 w-full max-w-6xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden"
                    onClick={(e) => e.stopPropagation()}
                    // แอนิเมชันสำหรับตัว Modal
                    initial={{ opacity: 0, scale: 0.95, y: 20 }} // ตอนเกิด: โปร่งใส, ย่อขนาด, เลื่อนลงนิดหน่อย
                    animate={{ opacity: 1, scale: 1, y: 0 }}    // ตอนแสดงผล: แสดงผลปกติ
                    exit={{ opacity: 0, scale: 0.95, y: 20 }}   // ตอนหายไป: กลับไปสถานะตอนเกิด
                    transition={{ type: 'spring', stiffness: 300, damping: 25 }} // ใช้สปริงเพื่อให้ดูนุ่มนวล
                >
                    {!loading && details && (
